...
<body class="bg-gray-50 dark:bg-slate-950 flex flex-col min-h-screen font-sans transition-colors duration-300">
    ...
    <section id="jak-to-funguje" class="py-24 bg-white dark:bg-slate-900 px-6 transition-colors">
        <div class="max-w-7xl mx-auto">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-extrabold text-gray-900 dark:text-white mb-4">Průběh spolupráce</h2>
                <p class="text-gray-500 dark:text-gray-400 text-lg mx-auto max-w-2xl">Transparentní proces od tvého zadání až po ostrý web.</p>
            ...
            <div class="grid md:grid-cols-5 gap-6 relative">
                <?php 
                $steps = [
                    ['1. Registrace', 'Vytvoříš si účet pro přístup k chatu.', 'user-plus'],
                    ['2. Zadání', 'Nahraješ požadavky, soubory do systému a domluvíme se na ceně.', 'upload'],
                    ['3. Testování', 'Web nahraji na link, kde si ho proklikáš.', 'monitor'],
                    ['4. Platba', 'Pokud vše sedí, zaplatíš přes QR kód.', 'credit-card'],
                    ['5. Předání', 'Nasadím web na tvůj hosting a pošlu kódy.', 'check-circle']
                ];
                foreach($steps as $index => $step): ?>
                <div class="flex flex-col items-center text-center p-4 group">
                    <div class="w-14 h-14 bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-blue-400 rounded-2xl flex items-center justify-center mb-6 shadow-sm border border-gray-100 dark:border-slate-700 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
                        <span class="font-bold text-lg"><?= $index + 1 ?></span>
                    </div>
                    <h3 class="font-bold text-gray-900 dark:text-white mb-2"><?= $step[0] ?></h3>
                    <p class="text-gray-500 dark:text-gray-400 text-sm"><?= $step[1] ?></p>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
    ...
    <section id="faq" class="py-24 bg-white dark:bg-slate-900 px-6 transition-colors">
        <div class="max-w-3xl mx-auto">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-extrabold text-gray-900 dark:text-white mb-4 tracking-tight">Často kladené otázky</h2>
                <p class="text-gray-500 dark:text-gray-400 text-lg">Vše, co potřebuješ vědět o WebGrade.</p>
            ...
            <div class="space-y-4">
                ...
                $faqs = [
                    ...
                    ["Pomůžeš mi i s nahráním na hosting?", "Ano, to je poslední krok naší spolupráce. Postarám se o zprovoznění na tvém hostingu, aby byl web dostupný na tvojí adrese."],
                    ["Jak rychle bude web hotový?", "Standardní školní weby obvykle zvládám dodat do 3–5 pracovních dnů."]
                ];
                foreach($faqs as $faq): ?>
                <div class="faq-item border border-gray-100 dark:border-slate-800 rounded-2xl overflow-hidden transition-all duration-300 hover:border-blue-200 dark:hover:border-blue-500/50">
                    <button class="faq-question w-full flex items-center justify-between p-6 text-left bg-gray-50/50 dark:bg-slate-800/50 hover:bg-white dark:hover:bg-slate-800 transition-colors group">
                        <span class="font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition"><?= $faq[0] ?></span>
                    ...
                    <div class="faq-answer hidden p-6 pt-0 bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-400 leading-relaxed text-sm">
                        <?= $faq[1] ?>
                    </div>
                </div>
                <?php endforeach; ?>
        ...
        document.querySelectorAll('.faq-question').forEach(button => {
            button.addEventListener('click', () => {
                ...
                if (!answer.classList.contains('hidden')) {
                    item.classList.add('ring-2', 'ring-blue-100', 'dark:ring-blue-900/30', 'border-blue-200', 'dark:border-blue-500/50');
                } else {
                    item.classList.remove('ring-2', 'ring-blue-100', 'dark:ring-blue-900/30', 'border-blue-200', 'dark:border-blue-500/50');
                }
            });
        });
    </script>
