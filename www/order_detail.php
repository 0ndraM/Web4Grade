<?php
   ...
   // 3. Logika pro Admina: Aktualizace stavu, ceny a ODKAZU NA WEB
   if ($role === 'admin' && isset($_POST['update_order'])) {
       $new_status = $_POST['status'];
       $new_price = $_POST['price'];
       $new_url = trim($_POST['web_url'] ?? ''); // Načtení nové URL
   ...
   // 6. Logika pro Admina: Smazání zakázky
if ($role === 'admin' && isset($_POST['delete_order'])) {
    // 1. Smazání příloh zakázky z disku
    $stmt = $pdo->prepare("SELECT file_path FROM order_files WHERE order_id = ?");
    $stmt->execute([$order_id]);
    ...
    // 2. Smazání souborů z chatu z disku
    $stmt = $pdo->prepare("SELECT file_path FROM messages WHERE order_id = ? AND file_path IS NOT NULL");
    $stmt->execute([$order_id]);
    $chatFiles = $stmt->fetchAll(PDO::FETCH_COLUMN);
    foreach ($chatFiles as $cf) {
        if (!empty($cf) && file_exists("./uploads/" . $cf)) unlink("./uploads/" . $cf);
    }

    // 3. Smazání z DB (Všechno ostatní se smaže automaticky díky ON DELETE CASCADE)
   ...
   // Výpočet údajů pro zobrazení v HTML (ponechej nahoře v PHP bloku)
      ...
      <div class="max-w-6xl mx-auto flex flex-col-reverse lg:grid lg:grid-cols-3 gap-4 md:gap-8">
         <div class="p-4 md:p-0 lg:col-span-1 space-y-4">
            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
               ...
               <div class="mb-6">
                  <p class="text-[10px] font-bold text-gray-400 uppercase mb-2">Vyhotovený web:</p>
                  <a href="<?= htmlspecialchars($order['web_url']) ?>" target="_blank" 
                     class="flex items-center justify-center gap-2 bg-green-600 text-white font-bold py-3 px-4 rounded-xl hover:bg-green-700 transition shadow-lg shadow-green-100">
                  <span>🌐</span> Otevřít hotový web
                  </a>
               </div>
               <?php endif; ?>
               <?php if (!empty($files)): ?>
            ...
            <div class="bg-yellow-50 p-5 rounded-xl border border-yellow-200 shadow-sm">
               <h3 class="font-bold mb-3 text-yellow-800 text-xs uppercase tracking-widest">Admin Panel</h3>
               <form method="POST" class="space-y-4">
                  <div>
                     <label class="block text-[10px] font-bold text-yellow-700 uppercase mb-1">Stav zakázky</label>
                  ...
                  <div>
                     <label class="block text-[10px] font-bold text-yellow-700 uppercase mb-1">Cena (Kč)</label>
                     <input type="number" name="price" value="<?= $order['price'] ?>" class="w-full border-gray-300 p-2 rounded-lg text-sm outline-none">
                  </div>
                  <div>
                     <label class="block text-[10px] font-bold text-yellow-700 uppercase mb-1">URL vyhotoveného webu</label>
                     <input type="url" name="web_url" value="<?= htmlspecialchars($order['web_url'] ?? '') ?>" 
                        placeholder="https://priklad.cz"
                        class="w-full border-gray-300 p-2 rounded-lg text-sm outline-none focus:ring-2 focus:ring-yellow-400">
                  </div>
               <form method="POST" class="space-y-4">
                  <button type="submit" name="update_order" class="w-full bg-yellow-600 text-white py-2 rounded-lg font-bold text-sm shadow-md hover:bg-yellow-700 transition">Aktualizovat vše</button>
               </form>
               <form method="POST" class="mt-6 pt-4 border-t border-yellow-200" onsubmit="return confirm('Opravdu chcete celou zakázku smazat? Tato akce je nevratná a smaže i všechny zprávy a soubory!')">
                  <button type="submit" name="delete_order" class="w-full bg-red-50 text-red-600 border border-red-200 py-2 rounded-lg font-bold text-xs hover:bg-red-600 hover:text-white transition">
                  🗑️ SMAZAT ZAKÁZKU
                  </button>
               </form>
            </div>
